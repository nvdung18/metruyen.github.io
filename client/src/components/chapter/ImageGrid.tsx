...
/** Helper to extract CID from stored url */
const getCid = (url: string) => {
  const parts = url.split('/');
  return parts[parts.length - 1];
};

/** Dedicated tile component so we can use hooks safely */
const GridTile = ({
  image,
  isSelected,
  isSelectionMode,
  ...
  onToggleSelection: (img: ChapterImage) => void;
  onConfirmDelete: (id: string) => void;
}) => {
  const cid = getCid(image.url);
  ...
  return (
    <div
      className={`group relative aspect-[2/3] cursor-pointer overflow-hidden rounded-md border transition-all duration-150 ease-in-out ${
        isSelected ? 'ring-primary scale-95 ring-2' : 'hover:scale-[1.02]'
      } ${isSelectionMode ? '' : 'hover:shadow-md'}`}
      onClick={() => onToggleSelection(image)}
      aria-label={`Page ${image.pageNumber}, ${isSelected ? 'selected' : 'click to select'}`}
    >
      ...
      {/* Actual image with gateway fallback */}
      <Image
      ...
      {!isSelected && <div className="absolute inset-0 bg-black/0 transition-colors group-hover:bg-black/10" />}
    </div>
  );
};
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between">
          ...
          {images.length > 0 && (
            <div className="text-muted-foreground text-right text-xs">
              {isSelectionMode ? 'Click image to toggle selection' : 'Click image to select for update'}
            </div>
          )}
        </div>
      </CardHeader>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {images.map((img) => (
              <GridTile
                key={img.id}
                image={img}
                isSelected={selectedImages.some((sel) => sel.id === img.id)}
